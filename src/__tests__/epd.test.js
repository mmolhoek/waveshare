...
const lgpio = __importStar(require("lgpio"));
// Mock lgpio library
jest.mock("lgpio", () => ({
...
describe("EPD Class", () => {
    let epd;
    beforeEach(() => {
        epd = new index_1.EPD({ debug: false });
    });
    afterEach(() => {
        jest.clearAllMocks();
        epd.cleanup();
    ...
    test("should clear the display buffer", async () => {
        const turnOnDisplaySpy = jest.spyOn(epd, "turnOnDisplay");
        await epd.clear();
...
//# sourceMappingURL=epd.test.js.map